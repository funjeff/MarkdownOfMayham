...
u32 mario_update_dash(struct MarioState *m){
    ...
    if (m->floor->type == SURFACE_DASH_PAD_DOWN_FAST){
     ...
     if (m->floor->type == SURFACE_DASH_PAD_FREEZE_DOWN){
    ...
    if (m->floor->type == SURFACE_DASH_PAD_FREEZE_UP){
        set_mario_action(m,ACT_DASH_NO_CONTROL, 0);
        m->forwardVel = 200;
        m->faceAngle [1] = 16340 * 3;
        if (!m->onPad){
    ...
    if (m->floor->type == SURFACE_DASH_PAD_FREEZE_LEFT){
        set_mario_action(m,ACT_DASH_NO_CONTROL, 0);
        m->forwardVel = 200;
        m->faceAngle [1] = 16340 * 2;
        if (!m->onPad){
    ...
    if (m->floor->type == SURFACE_DASH_PAD_FREEZE_RIGHT){
        set_mario_action(m,ACT_DASH_NO_CONTROL, 0);
        m->forwardVel = 200;
        m->faceAngle [1] = 16340 * 0;
        if (!m->onPad){
